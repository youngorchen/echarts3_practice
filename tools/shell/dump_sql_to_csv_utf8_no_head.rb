...
def dump_sql_result(client,sql)
	...
	res.each do |hash|
	 ...
	 # begin
	 	#no head
		#puts hash.map { |k,v| "#{conv.iconv(k.to_s.gsub(/,/,'，'))}" }.join(",") if $first == 0
		#puts hash.map { |k,v| "#{conv.iconv(v.to_s.gsub(/,/,'，'))}" }.join(",")
		# no convert
		puts hash.map { |k,v| "#{v.to_s.gsub(/,/,'，').gsub(/\n/,' ')}" }.join("\t")
		$first = 1
	 # rescue
	 # 	next
	 # end
...
def dump_sql_to_csv(host,database,sql_file,output_csv_file,extra)
	...
	#puts `ruby /data/scripts/mail.rb  a@b.com '测试数据' '详见附件' #{fn}`
end

unless ARGV[3]
	puts "host port db_name sql_file output_csv_file"
	exit
end
...
pp extra
dump_sql_to_csv(host,database,sql_file,output_csv_file,extra)
